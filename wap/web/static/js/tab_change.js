...
var floor_sel_txt = "";
var floor_sel_setup = -2;
var floor_up_number = 3;

function floor_fun () {
    //获取楼层数量地上和地下
    $.ajax({
        url:baseApiUrl + '/build-wap/get-build-floors?token='+token,
        data:{'id':shop_id},
        type:"POST",
		async:false,
        success:function (phpdata) {
            if(phpdata.status == 200){
                floor_sel_setup = '-'+phpdata.data.low_floor_number;
                floor_up_number = phpdata.data.floor_number;
            }else{
                $('.sy-installed-ts').text('未获取到楼层信息');
                tippanel();
            }
        },error:function () {
            $('.sy-installed-ts').text('获取楼层失败');
            tippanel();
        }
    });
	//生成楼层数量
	for (floor_sel_setup; floor_sel_setup < floor_up_number; floor_sel_setup++) {
		if (0 == floor_sel_setup) {
			continue;
		}
		floor_sel_txt += "<input class='floor_btns' type='button' value="+floor_sel_setup+">";
	}
	$('.floor_con').html(floor_sel_txt);
	floor_sel_txt = "";
}
function floor_init () {
	//重置楼层所有状态
...
$('#list_box .equipment_type').live('click',function () {//显示	
	...
	$('.equipment_list li').live('click',function () {
        ...
        self_me.find('.user_result').next().val($(this).attr('data-id'));
