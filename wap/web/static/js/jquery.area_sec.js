...
function selectP(id,name) {
    ...
    if(aa.length!=bb.length){
        ...
        if (djcs==4){
            ...
            // $("."+thwz).find(".yx_srnr_dqu").html(expressArea);
            next.html(expressArea);
            $(myfuji).find(".yx_srnr_dqu").css({"line-height":"16px","height":"35px","color":"#333"});
            $(myfuji).find(".yx_anzdz").css("height","90px");
            $(window).scrollTop(gdtwz);
            expressArea="";
            $.ajax({
                type: "GET",
...
function zxcs(djdiv,__this,fuji){
    gdtwz=$(window).scrollTop();
    thwz=djdiv;
    next = __this;
    myfuji=fuji
    $("#areaMask").fadeIn();
