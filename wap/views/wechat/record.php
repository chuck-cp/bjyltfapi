...
<script>
    ...
    $(function(){
        $.ajax({
            url:baseApiUrl+"/member/record",
            type:"get",
            dataType:"json",
            data:{token:token,wx_member_id:quan_wx_member_id,member_id:quan_member_id,page:page,wechat_id:wechat_id},
            success:function(data) {
                if(data.status==200&&data.data!=null){
                    ...
                    $.each(data.data,function(i,value){
                        ...
                        if(value.status==1){
                            html=html+"<div class='box-list'><div class='title'><p class='time'>"+value.create_at+"</p><p class='status'>"+statusa+"</p></div><div class='content'><a href='/shop/modify-shop?shop_id="+value.id+"&wechat_id=<?=$wx_member_id?>&token=<?=$token?>&type=weixin' class='detail'> <p>申请人：<span>"+value.apply_name+"</span></p><p>安装地址：<span>"+value.area_name+value.address+"</span></p><p>安装数量：<span>"+value.apply_screen_number+"</span></p><p>驳回原因：<span>"+value.examine_desc+"</span></p></a> </div></div>";
                        }else{
                            html=html+"<div class='box-list'><div class='title'><p class='time'>"+value.create_at+"</p><p class='status'>"+statusa+"</p></div><div class='content'><a href='/wechat/recordinfo?shopid="+value.id+"&token=<?=$token?>&wechat_id=<?=$wechat_id?>' class='detail'> <p>申请人：<span>"+value.apply_name+"</span></p><p>安装地址：<span>"+value.area_name+value.address+"</span></p><p>安装数量：<span>"+value.apply_screen_number+"</span></p></a> </div></div>";
                        }
                    });
