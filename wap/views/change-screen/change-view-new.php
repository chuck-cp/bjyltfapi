...
<form id="idform" >
    <div class="old_equipment" id="oldDv">
        <p class="old_equipment_tt">
            <span class="old_equipment_num">更换旧设备编号</span><span class="remarks_col">（选填）</span>
        </p>
    </div>
...
<!--        </div>-->
    ...
    <div class="old_equipment">
        ...
        <p><textarea class="wenti" rows="" id="descript" cols="" placeholder="请填写问题描述" nullmsg='problem_description' datatype='*'></textarea></p>
    </div>
    <br /><br /><br /><br /><br />
    <button class="submission sy-newinstalled" type="submit" data-v="0">完成安装</button>
</form>
<!--提交失败提示-->
...
<script type="text/javascript">
    ...
    $(function(){
        $.ajax({
            url:baseApiUrl+"/screeninstall/change-screen/<?=$replace_id?>?token=<?=$token?>",
            type:"get",
            dataType:"json",
            success:function(data) {
                //console.log(data);
                if(data.status==200 && data.data.replace_screen_number > 0){
                    var html="";
                    var oldDv = "";
                    var eachNumber = data.data.replace_screen_number;
                    $('#current').val(data.data.currentNumber);
                    for (var i=0; i<eachNumber; i++){
                        //1.old device number
                        oldDv=oldDv+"<div class='saomabox'>" +
                            "<span class='fsm'><input type='text' name='old' id='old"+i+"' value='' class='screen_text_list' placeholder='请扫描或填写设备硬件编号' /></span>"+
                            "<span><span onclick='scan(this)' class='sao_ma' data-type='old'>扫码</span></span>"+"<p class='red'>设备未在此店铺中找到</p>"+
                            "</div>";
                        //2. new device number
                        ...
                        if (/iphone|ipad|ipod/.test(ua)) {
                            html = html+'<div class="upload_box">' +
                                ...
                                '<input id="panorama'+i+'" name="newpic" class="update_input" type="hidden"  value="">' +
                                '<img class="imgspace" src="/static/images/up_img.png">' +
                                '<img class="addimg" src="/static/images/up_img.png">' +
                                ...
                                '</div>' +
                                '<div class="sao_ma_box">' +
                                '<p class="sao_ma_num fsm"><input type="text" name="new" id="newDv'+i+'" value="" class="screen_text_list" placeholder="请扫描或填写设备软件编号" /></p>' +
                                '<span class="tip" style="visibility: hidden">设备未在系统中找到或未出库</span>' +
                                '<span onclick="scan(this)" data-type="news" class="sao_ma sao_ma2">扫码</span>' +
                                '</div>' +
                                '</div>';
...
//            };
        ...
        beforeSubmit:function(curform){
            ...
            if(judedata==0){
                //旧的屏幕编码
                var remove_device_number = [];
                var remove_flag = 0;
                $('input[name="old"]').each(function (i) {
                    if(isInArray(remove_device_number,$(this).val())){
                        remove_flag = 1;
                        $('#old'+i+'').parent().next().next().html('设备编号不能重复').css('visibility','visible');
                        return false;
                    }
                    pushArr(remove_device_number,$(this).val());
                });
                if(remove_flag){return false;}
                //新的屏幕编码
                var install_software_number = [];
                var software_flag = 0;
                $('input[name="new"]').each(function (i) {
                    sofware = 1;
                    if(isInArray(install_software_number,$(this).val())){
                        software_flag = 1;
                        $('#newDv'+i+'').parent().next().html('设备编号不能重复').css('visibility','visible');
                        return false;
                    }
                    pushArr(install_software_number,$(this).val());
                });
                if(software_flag){return false;}
                //新增屏幕图片
                var new_images = [];
                $('input[name="newpic"]').each(function () {
                    pushArr(new_images,$(this).val());
                })
                if(remove_device_number.length != install_software_number.length){
                    conftankuangparam("sy_examin_panel",'新增屏幕数必须和拆除屏幕数一致');
                    return false;
                }
                ...
                //1首先验证屏幕合法性
                $.ajax({
                    url:baseApiUrl+"/screen-change/change-post-new-check",
                    type:"post",
                    dataType:"json",
                    ...
                    success:function (phpdata) {
                        if(phpdata.status == 200){
                            location.href="/screen/screenwait?shopid=<?=$shopid?>&token=<?=$token?>&change=change&replace_id=<?=$replace_id?>&operate=2";
