...
?>
<!DOCTYPE html>
<!-- saved from url=(0069)http://wap.bjyl.com/build?token=hljlYufb95guIx6UFbeX_4uNASHMVT9X&dev= -->
<html>
<head>
...
<ul class="park_apple" id="list_box">
    <li>
        ...
        <div class="biz_box">
            ...
            <a href="<?=Url::to(['/build/build-create','token'=>$token])?>">
    ...
    <li>
        ...
        <div class="biz_box" >
            <div class="equipment_type" id="poster_equipment"><img  src="/static/image/more_apply/poster_equipment.png" ></div>
            <div class="equipment_type" id="led_equipment"><img  src="/static/image/more_apply/led_equipment.png" ></div>
        </div>
    </li>
</ul>
<!-- 遮罩 -->
...
<script type="text/javascript">
    ...
    $('.biz_box .equipment_type').on('click',function () {
        ...
        $.ajax({
            url: baseApiUrl + "/build-wap/choose-buildings?token="+token,
            type: "POST",
            async: false,
            success:function (phpdata) {
                if(phpdata){
                    if(__this == 0){
                        sel_inc1 = phpdata.data;
                    }else{
                        sel_inc2 =phpdata.data;
                    }
                }else{
                    $('.sy-installed-ts').text('没有可选择的楼宇！');
                    tippanel();
                }
    ...
    function list_info(a,b) {
        ...
        $.each(a, function(index, value) {
            if(b == 1){
                sel_def += "<a href='/build/scene-post?token="+token+"&build_id="+index+"&screen_type=2&shop_type=1'><li>" + value + "</li></a>";
            }else if(b ==2){
                sel_def += "<a href='/build/scene-led?token="+token+"&build_id="+index+"&screen_type=1&shop_type=1'><li>" + value + "</li></a>";
            }
        });
    }
