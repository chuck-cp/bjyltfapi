...
<head>
    ...
    <link type="text/css" href="/static/css/mui.poppicker.css" rel="stylesheet" />
    <link type="text/css" href="/static/css/mui.picker.css" rel="stylesheet">
    <style type="text/css">
...
<script type="text/javascript">
    //读取配置和已经填写过的信息
    $.ajax({
        url: baseApiUrl + "/build-wap/build-bill-scenes?token="+token+'&build_id='+build_id+'&screen_type=2',
        type: "GET",
        async: true,
        success:function (phpdata) {
            ...
            if(data_length > 0){
                ...
                for(var i=0; i<data_length; i++){
                    if(phpdata.data[i].screen_number){//已保存过
                        addhtml += '<li>';
                        addhtml += '<div class="show_tt">';
                        addhtml += '<em>'+phpdata.data[i].position_name+'</em><span><a href="/build/detail-scene?token=<?=$token?>&screen_type=<?=$screen_type?>&shop_type=<?=$shop_type?>&id='+phpdata.data[i].id+'&shop_id='+build_id+'"><img class="building_add" src="/static/image/revise_btn.png" ></a></span>';
                        addhtml += '</div>';
