    ...
    <form id="idform">
        ...
        <div class="content">
            ...
            <div class="sy-newuploadimg">
                <p class="tit">请上传安装完成后的LED屏幕照片</p>
                <div class="con">
                </div>
            </div>
        </div>
...
<script>
    ...
    $(function(){
        $.ajax({
            //url:baseApiUrl+"/screeninstall/screenimgshow/<?=$shopid?>?token=<?=$token?>",
            url:baseApiUrl+"/screeninstall/change-update/<?=$shopid?>?token=<?=$token?>&replace_id=<?=$replace_id?>",
            type:"get",
            dataType:"json",
            success:function(data) {
                if(data.status==200 && data.data.replace.length > 0){
                    var tablehtml=" <table>";
                    if(data.data.install_member.team.length > 0){
                    ...
                    tablehtml=tablehtml+" <tr> " +
                    "<td>安装人姓名</td> " +
                    "<td>"+data.data.install_member.t_member.install_member_name+"</td> " +
                    "</tr> " +
                    "<tr> " +
                    "<td>手机号</td> " +
                    "<td>"+data.data.install_member.t_member.mobile+"</td> " +
                    "</tr> " +
                    "</table>";
...
//	  tiptype:3,
        ...
        beforeSubmit:function(curform){
            ...
            $.ajax({
                url:baseApiUrl+"/screeninstall/change-not-pass-update",
                type:"POST",
                dataType:"json",
                data:{replace_id:<?=$replace_id?>,install_images:install_images,token:'<?=$token?>'},
                success:function(data) {
                    if(data.status==200){
