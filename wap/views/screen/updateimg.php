...
<script>
    ...
    $(function(){
        $.ajax({
            url:baseApiUrl+"/screeninstall/underlineimgshow/<?=$id?>?token=<?=$token?>",
            type:"get",
            dataType:"json",
...
//	  tiptype:3,
        ...
        beforeSubmit:function(curform){
            ...
            $.ajax({
                ...
                type:"post",
                dataType:"json",
                data:{id:<?=$id?>,install_name:install_name,install_mobile:install_mobile,install_images:install_images,token:'<?=$token?>'},
                success:function(data) {
                    if(data.status==200){
                        setTimeout(function() {location.href="/screen/wait?number=<?=$id?>&apply_code=<?=$apply_code?>&dynamic_code=<?=$dynamic_code?>&token=<?=$token?>"},1000);
                    }else{
