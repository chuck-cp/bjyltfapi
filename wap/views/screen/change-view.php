    ...
    $(function(){
        $.ajax({
            ...
            success:function(data) {
                if(data.status==200 && data.data.replace!=null){
                    var tablehtml=" <table>";
                    if(data.data.install_member.team.length){
                    ...
                    tablehtml=tablehtml+" <tr> " +
                    "<td>安装人姓名</td> " +
                    "<td id='install_name'>"+data.data.install_member.t_member.install_member_name+"</td> " +
                    "</tr> " +
                    "<tr> " +
                    "<td>手机号</td> " +
                    "<td id='install_mobile'>"+data.data.install_member.t_member.mobile+"</td> " +
                    "</tr> " +
                    "</table>";
                    $(".sy_onecont_list").html(tablehtml);
                    $("#screenBB").val(JSON.stringify(data.data.replace));
                    var  html="";
                    $.each(data.data.replace,function(i,value){
    ...
    $("#idform").Validform({
        ...
        beforeSubmit:function(curform){
            ...
            if(judedata==0){
                ...
                $.ajax({
                    url:baseApiUrl+"/screeninstall/change-post",
                    type:"post",
                    dataType:"json",
                    data:{id:<?=$shopid?>,install_name:install_name,install_mobile:install_mobile,install_images:install_images,token:'<?=$token?>',isupdate:false, replace_id:<?=$replace_id?>},
                    success:function(data) {
                        if(data.status==200){
                            ...
                            if(data.status==484) {
                                ...
                                $('.sy-newinstalled').removeClass('set-grey');
                                $('.sy-newinstalled').attr('data-v',0);
                            }else if(data.status==674 ){
                                $('#sc'+data.data.id).find(".tip").html("");
                            }else{
                                //alert(data.message);
