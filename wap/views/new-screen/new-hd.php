...
<!--        <p style="background: #F0F0F0;">&nbsp;</p>-->
    ...
    <div class="yx_shehe_jg" id="update" style="display: none;">
        <a href="/screen/screenover?shopid=<?=$shopid?>&token=<?=$token?>&istrue=0&change=<?=$change?>&replace_id=<?=$replace_id?>&operate=<?=$operate?>" class="yx_shenghe_cheng">激活失败更换屏幕</a>
    </div>
    <!--修改安装信息-->
    <div class="yx_shehe_jg" id="weitongguo" style="display: none;">
        ...
        <?if($operate == 4):?>
            <a href="/screen/screen-new-update?shopid=<?=$shopid?>&token=<?=$token?>&dev=<?=$dev?>&replace_id=<?=$replace_id?>" class="yx_shenghe_cheng">修改安装信息</a>
        <?else:?>
            ...
            success:function(data) {
                ...
                if(data.status==200 && data.data!=null){
                    ...
                    for(var i=0; i<newdv.length; i++){
                        if(i == 0){
                            newHtml += '<div class="yx_sh_lb"><p class="lf_mc">新增设备编号</p><p class="rl_nr new_dv">'+newdv[i]+'</p></div>';
                        }else{
