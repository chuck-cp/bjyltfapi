    ...
    function settime(){
        if (countdown == 0) {
            ...
            $('#shuaxin').click(function(){
                $.ajax({
                    url:baseApiUrl+"/screeninstall/screenstatus/<?=$shopid?>?replace_id="+replace_id,
                    type:"get",
                    dataType:"json",
                    data:{token:'<?=$token?>'},
                    success:function(data) {
                        if(data.status){
                            if(data.data.status){
                                ...
                                if(change == 'change'){
                                    location.href="/screen/screenover?shopid=<?=$shopid?>&token=<?=$token?>&istrue=0&change=<?=$change?>&replace_id=<?=$replace_id?>&operate=<?=$operate?>";
                                }else{
                                    location.href="/screen/screenover?shopid=<?=$shopid?>&token=<?=$token?>&istrue=0&replace_id=<?=$replace_id?>&operate=<?=$operate?>";
                                }
                            }
                        }else{
            ...
            if(jiange>=6){
                ...
                $.ajax({
                    ...
                    success:function(data) {
                        if(data.status){
                            if (data.data.status) {
                                location.href = "/screen/screenover?shopid=<?=$shopid?>&token=<?=$token?>&istrue=1&operate=<?=$operate?>";
                                return false;
                            }
                        }
                    }
